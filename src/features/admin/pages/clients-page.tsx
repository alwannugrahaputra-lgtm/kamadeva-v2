          ...
          <div className="glass-card rounded-[30px] p-6">
            <div className="mb-5 flex items-center justify-between">
              <div>
                ...
                <h3 className="mt-2 text-2xl font-semibold text-[var(--brand-deep)]">
                  {editingClient ? editingClient.fullName : "Data klien baru"}
                </h3>
              </div>
              {editingClient ? (
                <Link href="/admin/clients" className="text-sm font-semibold text-[var(--brand-deep)]">
                  Reset form
                </Link>
              ) : null}
            </div>
            <form action={upsertClient}>
              ...
              <div className="form-grid">
                ...
                <input name="eventType" defaultValue={editingClient?.eventType ?? ""} placeholder="Jenis acara" className="input-base" required />
              ...
              <textarea
                name="specialNotes"
                defaultValue={editingClient?.specialNotes ?? ""}
                placeholder="Catatan kebutuhan khusus"
                className="input-base mt-4 min-h-32"
              />
              <div className="mt-4">
                <SubmitButton pendingText="Menyimpan klien...">
            ...
            <table>
              ...
              <tbody>
                {clients.map((client) => (
                  <tr key={client.id}>
                    <td>
                      <p className="font-semibold text-[var(--brand-deep)]">{client.fullName}</p>
                      <p className="text-sm text-[var(--muted)]">{client.phone}</p>
                    </td>
                    <td>
                      <p>{client.eventType}</p>
                      <p className="text-sm text-[var(--muted)]">{formatDate(client.eventDate)}</p>
                    </td>
                    <td>{client.selectedPackage?.name ?? "-"}</td>
                    <td>
                      <Badge tone={client.status === ClientStatus.SELESAI ? "success" : "brand"}>{client.status}</Badge>
                    </td>
                    <td>{formatCurrency(client.budget)}</td>
                    <td>
                      {canManageClients ? (
                        <div className="flex gap-2">
                          <Link href={`/admin/clients?edit=${client.id}`} className="rounded-full border border-[var(--line)] px-3 py-2 text-xs font-semibold">
                            Edit
                          </Link>

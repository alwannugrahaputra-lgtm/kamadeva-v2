  ...
  return (
    <section className="container-shell py-14">
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-3">
        {packages.map((pkg) => (
          <article key={pkg.id} className="glass-card rounded-[32px] p-6">
            <p className="text-sm uppercase tracking-[0.2em] text-[var(--muted)]">
              {pkg.recommendedVenue ?? "Fleksibel"}
            </p>
            <h2 className="mt-3 section-title text-4xl font-semibold text-[var(--brand-deep)]">
              {pkg.name}
            </h2>
            ...
            <div className="mt-5 rounded-[24px] bg-white p-4 text-sm leading-7 text-[var(--muted)]">
